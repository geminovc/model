...
# (fine-uning the Bilayer on a single video) with also trying freezing
# different parts of the Bilayer network
# After training, the script uses inference piepline to generate predicted
# unseen image and video session strips accordingly

run_per_video_experiments () {
    # arg1: name of the frozen networks
    # arg2: unfreezes the texture generator's last layers (after AdaSpade layer) if True
    # arg3: unfreezes the inference generator's last layers (after AdaSpade layer) if True
    # arg4: name of the experiment
    frozen_networks="$1"
...
do_training=false
# if with_frozen_last_layer_flag is true, experiments in which the last layer 
# of G_inf or G_tex is frozen are conducted
with_frozen_last_layer_flag=false
# if single_last_layer_flag is true, experiments in which only the last
# layer of G_inf or G_tex is unfrozen are conducted
single_last_layer_flag=false

# Run per_video experiments
