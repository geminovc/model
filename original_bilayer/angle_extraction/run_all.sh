...
do  
	...
	do  
		tmux new-session -d -s my_session_0 "${command_to_run} && python extract_angles.py --snapshot ${model_path} --gpu ${c} --proc $((num_gpus*num_threads_per_gpu)) --index $((c*num_threads_per_gpu + k)) --root ${dataset_path} --save_path ${save_path}"
	done
done
