...
def time_generator(model):
    ...
    for _ in range(WARM_UP):
        _, _ = model(x0, {'value':x1, 'jacobian':x2}, {'value':x3, 'jacobian':x4})
    
    # measuring
    total_times = []
    times_dict = {'dense_motion_time':[], 'first_time': [], 'down_blocks_time': [],
                  'bottleneck_time': [], 'up_blocks_time': [], 'final_time': []}
    with torch.no_grad():
        for rep in range(NUM_RUNS):
            print(rep)
            starter.record()
            _, time_dict =  model(x0, {'value':x1, 'jacobian':x2}, {'value':x3, 'jacobian':x4})
            ender.record()
            # WAIT FOR GPU SYNC
