...
parser.add_argument("--print_macs", dest="print_macs", action="store_true", help="print_macs")
parser.add_argument("--netadapt_checkpoint", default=None, help="path to netadapt checkpoint")
...
generator, discriminator, kp_detector = configure_fom_modules(config, device)
if PRINT_MACS:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        start = (get_decode_and_bottleneck_macs('log', deepcopy(generator),
                                                kp_detector, device,
                                                LR_SIZE, IMAGE_RESOLUTION))
    print("START", start)
...
if reload_gen is not None:
    ...
    if PRINT_MACS:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            end = get_decode_and_bottleneck_macs('log', deepcopy(generator),
                                                 kp_detector, device,
                                                 LR_SIZE, IMAGE_RESOLUTION)
        print("END", end)
