...
def get_frame_from_video_codec(frame_tensor, nr_list, dr_list, quantizer,
    ...
    for i, (frame, nr, dr) in enumerate(zip(frame_data, nr_list, dr_list)):
        ...
        jitter_frame = JitterFrame(data=b"".join(payload_data),
                                   timestamp=timestamp)
        decoded_frames = decoder.decode(jitter_frame)
        decoded_frame = decoded_frames[0].to_rgb().to_ndarray()
        decoded_data[i] = np.transpose(decoded_frame,
...
def train(config, generator, discriminator, kp_detector, checkpoint, log_dir,
    ...
    if dense_motion_params.get('concatenate_lr_frame_to_hourglass_output',
                               False):
    ...
    if config['model_params']['discriminator_params'].get(
    ...
    if checkpoint is not None and generator_type in [
            "occlusion_aware", "split_hf_lf"
    ]:
        if train_params.get('fine_tune_entire_model', False):
            start_epoch = Logger.load_cpk(
                ...
                None if train_params['lr_kp_detector'] == 0 else
    ...
    if train_params.get('train_only_generator',
                        False) and checkpoint is not None:
        if kp_detector is not None:
            ...
            for param in generator.dense_motion_network.parameters():
                param.requires_grad = False
    elif train_params.get('train_everything_but_generator',
                          False) and checkpoint is not None:
    ...
    if train_params.get('train_only_non_fom_layers',
                        False) and checkpoint is not None:
        if kp_detector is not None:
            for param in kp_detector.parameters():
                param.requires_grad = False
        ...
        for name, param in generator.named_parameters():
            if not (name.startswith("sigmoid") or name.startswith("hr")
                    ...
                    for i, y in enumerate(metrics_dataloader):
                        if use_lr_video or use_RIFE:
                            ...
                            if train_params.get('encode_video_for_training',
                                ...
                                nr = y.get(
                                    'time_base_nr',
                                    torch.ones(lr_frame.size(dim=0),
                                               dtype=int))
                                dr = y.get(
                                    'time_base_dr',
                                    30000 * torch.ones(lr_frame.size(dim=0),
                                                       dtype=int))
