...
def train(
    ...
    use_lr_video = (
        generator_params.get("use_lr_video", False)
        or generator_type == "just_upsampler"
    )
    lr_size = generator_params.get("lr_size", 64)

    lr_video_locations = []
    dense_motion_params = generator_params.get("dense_motion_params", {})
    if dense_motion_params.get(
        "concatenate_lr_frame_to_hourglass_input", False
    ...
    if (
        generator_params.get("use_3_pathways", False)
        or generator_params.get("concat_lr_video_in_decoder", False)
        or generator_type == "just_upsampler"
    ):
    ...
    if train_params.get("train_only_generator", False) and checkpoint is not None:
        if kp_detector is not None:
            ...
            assert (
                ev_loss == 0 and ev_jacobian == 0
            ), "Equivariance losses must be 0 to freeze keypoint detector"

            for param in generator.dense_motion_network.parameters():
                param.requires_grad = False
    elif (
        train_params.get("train_everything_but_generator", False)
    ...
    if train_params.get("train_only_non_fom_layers", False) and checkpoint is not None:
        ...
        for name, param in generator.named_parameters():
            if not (
                name.startswith("sigmoid")
                or name.startswith("hr")
    ...
    if reload_gen is not None:
        ...
        optimizer_kp_detector = torch.optim.Adam(
            generator_full.kp_extractor.parameters(),
            lr=train_params["lr_kp_detector"],
            betas=(0.5, 0.999),
        )
        optimizer_discriminator = torch.optim.Adam(
            generator_full.discriminator.parameters(),
            lr=train_params["lr_discriminator"],
            betas=(0.5, 0.999),
        )
        ...
        discriminator_full = DiscriminatorFullModel(
            generator_full.kp_extractor,
            generator_full.generator,
            generator_full.discriminator,
            train_params,
        )
                    ...
                    with torch.no_grad():
                        for i, y in enumerate(metrics_dataloader):
                            ...
                            (
                                losses_generator,
                                metrics_generated,
                            ) = generator_full_parallel(y, generator_type)
                            losses = {
            ...
            for x in dataloader:
                if use_lr_video or use_RIFE:
                    ...
                    if train_params.get("encode_video_for_training", False):
                        ...
                        nr = x.get(
                            "time_base_nr", torch.ones(lr_frame.size(dim=0), dtype=int)
                        )
                        dr = x.get(
                            "time_base_dr",
                            30000 * torch.ones(lr_frame.size(dim=0), dtype=int),
                        )
                        x["driving_lr"] = get_frame_from_video_codec(
                            lr_frame, nr, dr, quantizer_level, target_bitrate, codec
                        )
                    else:
                    ...
                    for i, y in enumerate(metrics_dataloader):
                        if use_lr_video or use_RIFE:
                            ...
                            if train_params.get("encode_video_for_training", False):
                                ...
                                y["driving_lr"] = get_frame_from_video_codec(
                                    lr_frame, nr, dr, quantizer_level, target_bitrate
                                )
                            else:
                                y["driving_lr"] = lr_frame
