...
def select_convs(model, params):
    ...
    names = [m for n, m in model.named_modules() if (isinstance(m, nn.Conv2d) or isinstance(m, first_order_model.sync_batchnorm.SynchronizedBatchNorm2d))]
...
def calculate_macs(model, file_name):
    ...
    def follow(node, results, shape, times):
        ...
        if node.type == 'conv':
            temp_input_cpu = torch.rand(1,node.value.weight.shape[1], new_shape_[0], new_shape_[1])
        else:
            temp_input_cpu = torch.rand(1,node.value.weight.shape[0], new_shape_[0], new_shape_[1])
...
def channel_prune(model, prune_ratio, deletions = None):
    ...
    if deletions is None:
        ...
        for pruners in deletions:
            if pruners[0] == 'first':
                ...
                if node.type == 'conv' and len(node.after) != 0:
                    # Prune the outupts
                    node.value.weight.set_(node.value.weight.detach()[:pruners[1][0]])
                    node.value.bias.set_(node.value.bias.detach()[:pruners[1][0]])
                else:
                    print("FUCK")
            else:
                for i in range(len(pruners)):
        ...
        for base_index in layer_graph:
            ...
            curr_index = 0
            for node_index in node.before:
                total_rows += int(prune_ratios[node_index] * layer_graph[node_index].o)
                pruners.append((int(counter + (layer_graph[node_index].o * prune_ratios[node_index])), counter + (layer_graph[node_index].o)))
                counter += layer_graph[node_index].o
...
def reduce_macs(model, target, current):
    ...
    for layer in layer_graph:
        ...
        for following_layer in following_layers:
            ...
            for previous_layer in layer_graph[following_layer].before:
                for deletion in deletions[previous_layer]:
                    deletions[following_layer].append((counter + deletion[0], counter + deletion[1]))
        ...
        channel_prune(model, 0.1, deletions) # The 0.1 is a dummy variable for now, gets ignored
